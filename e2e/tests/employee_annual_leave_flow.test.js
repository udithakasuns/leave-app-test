...
describe('Employee Tests', () => {
    ...
    const casualFullLeaveDate = moment().startOf('month').add(2, 'week').day(1)
    const casualFirstHalfLeaveDate =  moment().startOf('month').add(2, 'week').day(2)
    const casualSecondHalfLeaveDate = moment().startOf('month').add(2, 'week').day(3)
    const casualCancelLeaveDate = moment().startOf('month').add(2, 'week').day(4)
    const casualUndoLeaveDate =  moment().startOf('month').add(2, 'week').day(5)
    ...
    it('Should be able to apply for a full day annual leave', async () => {
        ...
        const index = await homeScreen.getLeaveRequestIndexFromList(getFormattedDate(casualFullLeaveDate, "Do MMM"), 'Annual', 'Pending');
    ...
    it('Should be able to undo a annual leave request', async () => {
        ...
        const index = await homeScreen.getLeaveRequestIndexFromList(getFormattedDate(casualUndoLeaveDate, "Do MMM"), 'Annual', 'Cancelled');
        homeScreen.assertLeaveRequestExist(index);
    });

    it('Should be able to apply for a first half annual leave', async () => {
        await homeScreen.tapApplyLeave();
        await homeScreen.selectLeaveType(LeaveType.ANNUAL);
        ...
        await homeScreen.verifyLeaveRequestConfirmed();
        await homeScreen.tapProceedHome();
        const index = await homeScreen.getLeaveRequestIndexFromList(getFormattedDate(casualFirstHalfLeaveDate, "Do MMM"), 'Annual', 'Pending');
        homeScreen.assertLeaveRequestExist(index);
    });

    it('Should be able to apply for a second half annual leave', async () => {
        await homeScreen.tapApplyLeave();
        await homeScreen.selectLeaveType(LeaveType.ANNUAL);
        await homeScreen.selectLeaveDuration(LeaveDuration.HALF_DAY);
        ...
        await homeScreen.verifyLeaveRequestConfirmed();
        await homeScreen.tapProceedHome();
        const index = await homeScreen.getLeaveRequestIndexFromList(getFormattedDate(casualSecondHalfLeaveDate, "Do MMM"), 'Annual', 'Pending');
        homeScreen.assertLeaveRequestExist(index);
    });
    ...
    it('Should be able to cancel a applied annual leave', async () => {
        ...
        await homeScreen.verifyLeaveRequestConfirmed();
        await homeScreen.tapProceedHome();
        let index = await homeScreen.getLeaveRequestIndexFromList(getFormattedDate(casualCancelLeaveDate, "Do MMM"), 'Annual', 'Pending');
        await homeScreen.cancelLatestLeaveRequestByIndex(index);
        index = await homeScreen.getLeaveRequestIndexFromList(getFormattedDate(casualCancelLeaveDate, "Do MMM"), 'Annual', 'Cancelled');
        homeScreen.assertLeaveRequestExist(index);
    });
});
